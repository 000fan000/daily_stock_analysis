...
| MA250 | {today.get('ma250', 'N/A')} | 超长期趋势线 |
...
| 乖离率(MA60) | {trend.get('bias_ma60', 0):+.2f}% | 长期趋势偏离度 |
| 乖离率(MA250) | {trend.get('bias_ma250', 0):+.2f}% | 超长期趋势偏离度 |
...
| 指标 | 数值 | 信号 |
|------|------|------|
| K值 | {trend.get('kdj_k', 0):.1f} | |
| D值 | {trend.get('kdj_d', 0):.1f} | |
| J值 | {trend.get('kdj_j', 0):.1f} | |
| KDJ信号 | {trend.get('kdj_signal', '未知')} | |

#### 布林带分析
| 指标 | 数值 | 解读 |
|------|------|------|
| 上轨 | {trend.get('bb_upper', 0):.2f} | 压力位 |
| 中轨 | {trend.get('bb_middle', 0):.2f} | MA20 |
| 下轨 | {trend.get('bb_lower', 0):.2f} | 支撑位 |
| 带宽 | {trend.get('bb_width', 0):.1f}% | 波动率指标 |
| 价格位置 | {trend.get('bb_position', '未知')} | |

#### 动量指标分析
| 指标 | 数值 | 趋势 |
|------|------|------|
| 5日动量 | {trend.get('momentum_5d', 0):+.2f}% | 短期动能 |
| 10日动量 | {trend.get('momentum_10d', 0):+.2f}% | 中期动能 |
| 动量信号 | {trend.get('momentum_signal', '未知')} | |

#### 量均线分析
| 指标 | 数值 | 状态 |
|------|------|------|
| 5日量均 | {trend.get('vol_ma5', 0):,.0f} | |
| 10日量均 | {trend.get('vol_ma10', 0):,.0f} | |
...
- **技术指标综合评价**：基于均线、KDJ、布林带、动量、量能的综合判断
- **检查清单**：每项用 ✅/⚠️/❌ 标记，包含新增的技术指标检查
